...
def save_data_sqlite(df):
    ...
    '''
    try:
        conn = sqlite3.connect(r"C:\Users\alycamilos\Documents\MBA\Disciplina-Python-for-Data-Engineer\Projeto2\db-pipeline\data\NyflightsDB.db")
...
def fetch_sqlite_data(table):
    ...
    try:
        ...
        # Criação do cursor e execução da consulta
        c = conn.cursor()
        ...
        if 'conn' in locals() and conn:
            conn.close()
            logger.info(f'Conexão com banco fechada ; {datetime.datetime.now()}')
